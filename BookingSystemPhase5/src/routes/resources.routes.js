...
router.post("/", resourceValidators, async (req, res) => {
  ...
  if (!errors.isEmpty()) {
    ...
    const errMsg = errors.array().map(e => `${e.path}: ${e.msg}`).join("; ");
    await logEvent({
      actorUserId: null,
      message: `RESOURCE_CREATE_FAILED_VALIDATION: ${errMsg}`,
      entityType: "resource",
      entityId: null,
    });
  ...
  try {
    ...
    return res.status(201).json({ 
      ok: true, 
      data: rows[0],
      message: "Resurssi lisätty onnistuneesti! Tarkista lista."
    });
  } catch (err) {
    ...
    if (err && err.code === "23505") {
      ...
      return res.status(409).json({
        ok: false,
        error: "Resurssi on jo olemassa",
        details: "Kokeile muuta nimeä tai tarkista lista."
      });
    }
    ...
    return res.status(500).json({ 
      ...
      details: "Yritä hetken päästä uudelleen."
    });
  }
});
